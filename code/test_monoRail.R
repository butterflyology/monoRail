...
train_data <- tibble(Time = integer(), Train = integer(), Position = numeric())
for (t in 1:simulation_time) {
  ...
  for (i in 1:num_trains) {
    ...
    total_passengers_moved <- total_passengers_moved + train$passengers_moved
    train$passengers_moved <- 0  # Reset passengers_moved count for the next iteration
    
    # Append data to tibble
    train_data <- train_data %>% add_row(Time = t, Train = i, Position = train$position)
  }
